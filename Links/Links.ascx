<%@ Control Language="C#" AutoEventWireup="true" CodeFile="Links.ascx.cs" Inherits="DesktopModules_WET_Links_Links" %>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            var uploader = new qq.FineUploader({
                ...
                request: {
                    endpoint: '/DesktopModules/WET/Upload/Upload.aspx?FilePath=/Portals/<%= PortalId %>/Documents/Uploaded/<%= ModuleId %>/<%= System.Globalization.CultureInfo.CurrentUICulture.Name.Substring(0,2) %>/'
                },
                text: {
                ...
                callbacks: {
                    onComplete: function (id, fileName, responseJSON) {
                        if (responseJSON.success) {
                            var thislink = '/Portals/<%= PortalId %>/Documents/Uploaded/<%= ModuleId %>/<%= System.Globalization.CultureInfo.CurrentUICulture.Name.Substring(0,2) %>/' + responseJSON.msAddOn + fileName;
                            AddWetDocument('<%= ModuleId %>', '<%= System.Globalization.CultureInfo.CurrentUICulture.Name.Substring(0,2) %>', fileName.substring(0, fileName.lastIndexOf(".")), thislink, getLastOrder('<%= ModuleConfiguration.ModuleID%>'),'<%= PortalAlias.HTTPAlias %>','<%= Localization.GetString("msgEnterText.Text", "/DesktopModules/WET/Links/App_LocalResources/Links") %>');
                        }
                    }
                }
            });
            ...
            $('#<%= btnDeleteLink.ClientID %>').click(function (e) {
                e.preventDefault();
                var r = confirm('<%= Localization.GetString("msgDelete.Text", "/DesktopModules/WET/Links/App_LocalResources/Links")%>');
                if (r == true) {
                    var thisid = $('#<%= txtHiddenID.ClientID %>').text();
                    $.ajax({
                        url: "/DesktopModules/WET/Links/Save.asmx/Delete",
                        data: "{ 'itemid': '" + thisid +
                           "' }",
                        dataType: "json",
                        type: "POST",
            ...
            $(document).on('click', '.wet-list-edit<%= ModuleId %>', function (e) {
                ...
                $('#<%= txtName.ClientID %>').val($('#' + thisid).text());
                $('#<%= txtLink.ClientID %>').val($('#' + thisid).parent().attr("href"));
            ...
            $('#<%= btnSaveLink.ClientID %>').click(function (e) {
                e.preventDefault();
                if ($('#<%= txtLink.ClientID %>').val().indexOf("www.") == 0) {
                    $('#<%= txtLink.ClientID %>').val("http://" + $('#<%= txtLink.ClientID %>').val());
                }
                var order = $('#<%= txtOrder.ClientID %>').val();
                $.ajax({
                    url: "/DesktopModules/WET/Links/Save.asmx/Update",
                    data: "{ 'itemid': '" + $('#<%= txtHiddenID.ClientID %>').text() +
                        "','Title': '" + $('#<%= txtName.ClientID %>').val().replaceAll("'", "&#39;") +
                        "','Link': '" + $('#<%= txtLink.ClientID %>').val().replaceAll("&", "").replaceAll("#", "") +
                        "','Order': '" + order +
                        "' }",
                    dataType: "json",
                    type: "POST",
                    ...
                    $('#textitem' + $('#<%= txtHiddenID.ClientID %>').text()).html($('#<%= txtName.ClientID %>').val());
                    $('#textitem' + $('#<%= txtHiddenID.ClientID %>').text()).parent().attr("href", $('#<%= txtLink.ClientID %>').val());
...
<ul class="nav nav-pills nav-stacked wet-list">
    ...
    <div class="container-fluid">
        <div class="row center-block form-horizontal mrgn-tp-md" >
            <div class="form-group">
                <asp:label runat="server" resourcekey="lblName.Text" CssClass="control-label col-md-4" AssociatedControlID="txtName"></asp:label>
                <asp:textbox runat="server" ID="txtName" CssClass="form-control col-md-8"></asp:textbox>
            </div>
            <div class="form-group">
                <asp:label runat="server" resourcekey="lblLink.Text" CssClass="control-label col-md-4" AssociatedControlID="txtLink"></asp:label>
                <asp:textbox runat="server" ID="txtLink" CssClass="form-control col-md-8"></asp:textbox>
            </div>
            <div class="form-group">
                <asp:label runat="server" resourcekey="lblOrder.Text" CssClass="control-label col-md-4" AssociatedControlID="txtOrder"></asp:label>
