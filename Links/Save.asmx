<%@ WebService Language="C#" Class="Link_Save" %>
...
[ScriptService()]
public class Link_Save : System.Web.Services.WebService
{
	...
	public string AddNew(string Locale, int ModuleID, string Title, string Link)
	{
        ...
        if (Link.ToLower().StartsWith("www.")) { Link = Link.ToLower().Replace("www.", "http://www."); }
    ...
    [WebMethod()]
    public string Update(int itemid, string Title, string Link, string Order)
    {
        string returnval = "";
        ConnString.Open();
        SqlCommand WETCommand = new SqlCommand();
        ...
        if (Order == "-1")
        {
            WETCommand.CommandText = "UPDATE WET_List SET LinkName = @Title, LinkURL = @Link WHERE ID = @ItemID";
        }
        else
        {
            WETCommand.CommandText = "UPDATE WET_List SET LinkName = @Title, LinkURL = @Link, ListOrder = @Order WHERE ID = @ItemID";
